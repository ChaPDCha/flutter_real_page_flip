...
class _PremiumDemoScreenState extends State<PremiumDemoScreen>
  ...
  static const String _captureMode = String.fromEnvironment('RPF_CAPTURE_MODE');
  static const bool _isCaptureRun = _captureMode != '';
  ...
  void initState() {
    ...
    _dragAnimationController = AnimationController(
      vsync: this,
      duration: const Duration(milliseconds: _isCaptureRun ? 1900 : 1500),
    );
  ...
  Future<void> _runCaptureSequence() async {
    debugPrint('[CAPTURE] started mode=$_captureMode profile=high');
    ...
    debugPrint('[CAPTURE] finished mode=$_captureMode');
  }
  ...
  Widget _buildBookTheatre() {
    final Widget pageFlipWidget = PageFlipWidget(
      ...
      itemBuilder: (context, index) => _isCaptureRun && _isDoubleSpread
          ? _buildCaptureSpread(index)
          : _buildBookPage(index),
    );
  ...
  Widget _buildCaptureSpread(int spreadIndex) {
    final leftPage = spreadIndex * 2 + 1;
    final rightPage = leftPage + 1;
    return DecoratedBox(
      decoration: const BoxDecoration(color: Color(0xFF11111D)),
      child: Row(
        children: [
          Expanded(
            child: _buildCapturePage(
              pageNumber: leftPage,
              eyebrow: 'CHAPTER ${spreadIndex + 1}',
              title: const [
                'Paper in motion',
                'Natural shadows',
                'Responsive geometry',
          ...
          Expanded(
            child: _buildCapturePage(
              ...
              eyebrow: 'REAL PAGE FLIP',
              title: const [
                'Every fold has weight',
                'Depth without a 3D scene',
                'Smooth on real devices',
                'One gesture, two pages',
      ...
      Container(
        ...
        decoration: BoxDecoration(
          gradient: LinearGradient(
            ...
            colors: const [Color(0xFF171526), Color(0xFF0E0E18)],
          ),
        ),
        child: Column(
          crossAxisAlignment:
          ...
          children: [
            ...
            SizedBox(
              width: 360,
              child: Text(
                'A high-fidelity page transition with curved geometry, '
                'layered lighting, and a stable reading surface.',
                textAlign: alignRight ? TextAlign.right : TextAlign.left,
                style: TextStyle(
            ...
            Row(
              ...
              children: [
                Container(width: 34, height: 2, color: accent),
                const SizedBox(width: 12),
                Text(
                    ...
                    onChanged: (v) {
                      setState(() {
                        ...
                        if (_flapContentRevealEnd < _flapContentRevealStart) {
                          _flapContentRevealEnd =
                              _flapContentRevealStart + 0.02;
                        }
                      });
                    },
                  ),
                    ...
                    onChanged: (v) {
                      setState(() {
                        ...
                        if (_flapContentRevealStart > _flapContentRevealEnd) {
                          _flapContentRevealStart =
                              _flapContentRevealEnd - 0.02;
                        }
                      });
                    },
                  ),
