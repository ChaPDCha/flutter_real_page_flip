...
void main() {
  ...
  group('EpubPagingCalculator - Senior Edge Case Tests', () {
    ...
    testWidgets('Whitespace gaps do not produce blank pages', (
      WidgetTester tester,
    ) async {
      const text = 'A\n\n\n\n\n\n\n\n\n\n\n\n\n\nB';
    ...
    testWidgets('Short text fits into a single page', (
      WidgetTester tester,
    ) async {
    ...
    testWidgets('Long text is split into multiple pages', (
      WidgetTester tester,
    ) async {
      final buffer = StringBuffer();
      for (int i = 0; i < 30; i++) {
        buffer.writeln(
          'Paragraph $i. Flutter is a premium SDK for cross-platform apps.',
        );
      }
    ...
    testWidgets('Larger font size increases page count', (
      WidgetTester tester,
    ) async {
      final buffer = StringBuffer();
      for (int i = 0; i < 15; i++) {
    ...
    testWidgets(
      'Tries to break at space or newline boundary instead of mid-word',
      (WidgetTester tester) async {
        const text =
            'WordOne WordTwo WordThree WordFour WordFive WordSix WordSeven WordEight';
        ...
        for (final page in pages) {
          expect(
            ...
            isTrue,
            reason:
                'Page "$page" should start with Word to respect boundaries.',
          );
        }
      },
    );
    ...
    testWidgets('Extreme Viewport Constraint (No Infinite Loop)', (
      WidgetTester tester,
    ) async {
    ...
    testWidgets('Unicode Surrogate Pair / Emoji Integrity Protection', (
      WidgetTester tester,
    ) async {
      ...
      for (final page in pages) {
        ...
        for (int i = 0; i < page.length; i++) {
          ...
          if (codeUnit >= 0xD800 && codeUnit <= 0xDBFF) {
            expect(
              i + 1,
              lessThan(page.length),
              reason: 'High surrogate found at end of page: broken emoji!',
            );
    ...
    testWidgets('High-Load Performance Benchmark Stress Test', (
      ...
      stopwatch.stop();
      debugPrint(
        'Benchmark: Split ${text.length} chars into ${pages.length} pages in ${stopwatch.elapsedMilliseconds}ms',
      );
      ...
      expect(
        ...
        lessThan(2000),
        reason:
            'Paging calculator took too long: ${stopwatch.elapsedMilliseconds}ms',
      );
    });
  });
}
