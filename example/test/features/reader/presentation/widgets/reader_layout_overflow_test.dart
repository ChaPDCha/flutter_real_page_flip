...
List<EpubChapter> _chaptersWithLongTitle() {
  return [
    EpubChapter()..Title = '제1장 매우 긴 장 제목이 좁은 화면에서도 하단 바 레이아웃을 깨뜨리지 않아야 합니다',
  ];
}
...
void main() {
  group('reader chrome overflow', () {
    ...
    testWidgets(
      'ReaderBottomBar fits page, title, and chapter on narrow width',
      (tester) async {
        final book = _longTitleBook();
        final state = ReaderState(
          book: book,
    ...
    testWidgets('ReaderBottomBar fits chapter nav with many chapters', (
      tester,
    ) async {
      final book = _longTitleBook();
      final state = ReaderState(
        book: book,
        pages: List.filled(500, 'page'),
        chapters: List.generate(50, (i) => EpubChapter()..Title = 'Chapter $i'),
        isLoading: false,
    ...
    testWidgets(
      'ReaderBottomBar fits PDF title and page counter on narrow width',
      (tester) async {
