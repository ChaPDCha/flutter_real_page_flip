...
List<int> createMinimalEpubBytes({
...
<body><p>Hello Test World</p></body>
  ...
  archive.addFile(
    ArchiveFile('OEBPS/chapter1.html', chapterData.length, chapterData),
  );
...
void main() {
  ...
  group('BookshelfController Tests', () {
    ...
    test(
      'importEpub successfully copies file, parses metadata and saves to repo',
      () async {
    ...
    test(
      'importEpub handles parsing errors gracefully and sets AsyncError state',
      () async {
        when(() => mockRepository.getBooks()).thenAnswer((_) async => []);
    ...
    test(
      'removeBook deletes files from disk and removes from repository',
      () async {
