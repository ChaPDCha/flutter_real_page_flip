...
void main() {
  ...
  group('getLocalHighlightsDelta', () {
    test('returns highlights updated after the given timestamp', () async {
      ...
      final updatedAt = DateTime(2024, 6, 15, 10, 0, 0).toUtc();
      await db.into(db.highlights).insert(HighlightsCompanion.insert(
        bookId: 'book-1',
        chapterIndex: 0,
        startOffset: 10,
        endOffset: 20,
    ...
    test('returns empty list when no highlight was updated after since',
        () async {
      await db.into(db.books).insert(BooksCompanion.insert(
        id: 'book-1',
        title: 'Test Book',
  ...
  group('getLocalBookmarksDelta', () {
    ...
    test('returns empty list when no bookmark was updated after since',
        () async {
      await db.into(db.books).insert(BooksCompanion.insert(
        id: 'book-1',
        title: 'Test Book',
    ...
    test('handles bookmark with null updatedAt by using current time',
        () async {
      await db.into(db.books).insert(BooksCompanion.insert(
        id: 'book-1',
        title: 'Test Book',
  ...
  group('mergeRemoteBooks', () {
    test('inserts new book from remote data when local does not exist',
        () async {
      final remoteBook = {
        'id': 'remote-1',
        'title': 'Remote Book',
    ...
    test('updates existing book when remote timestamp is newer (LWW)',
        () async {
  ...
  group('mergeRemoteHighlights', () {
    ...
    test('inserts new highlight from remote data when local does not exist',
        () async {
    ...
    test('highlights are matched by bookId + chapterIndex + startOffset',
        () async {
      ...
      final firstHighlight =
          rows.firstWhere((h) => h.startOffset == 10);
      expect(firstHighlight.selectedText, 'first updated');
      final secondHighlight =
          rows.firstWhere((h) => h.startOffset == 30);
  ...
  group('mergeRemoteBookmarks', () {
    ...
    test('inserts new bookmark from remote data when local does not exist',
        () async {
    ...
    test('bookmarks are matched by bookId + chapterIndex + pageIndex',
        () async {
      await db.into(db.bookmarks).insert(BookmarksCompanion.insert(
        bookId: 'book-1',
        chapterIndex: 0,
