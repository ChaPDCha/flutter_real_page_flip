...
void main() {
  ...
  setUp(() async {
    ...
    when(() => mockClient.signInAnonymously()).thenAnswer((_) async => 'mock-user-123');
    ...
    when(() => mockRepo.getLocalHighlightsDelta(any())).thenAnswer((_) async => []);
    when(() => mockRepo.getLocalBookmarksDelta(any())).thenAnswer((_) async => []);
  ...
  test('SyncController initializes in idle state with correct timestamp anchor', () {
    final state = container.read(syncControllerProvider);
    expect(state.status, SyncStatus.idle);
    expect(state.lastSyncedAt?.millisecondsSinceEpoch, 1000);
  });

  test('SyncController silent anonymous sign-in and bidirectional sync flow succeeds', () async {
    final controller = container.read(syncControllerProvider.notifier);

    // Prepare delta payload mock assets
    final mockBookDelta = [{'id': 'book-1', 'title': 'Cloud Title'}];
    final mockHighlightDelta = [{'id': 2, 'book_id': 'book-1', 'selected_text': 'Warm golden highlight'}];

    when(() => mockClient.pullBooks(any())).thenAnswer((_) async => mockBookDelta);
    when(() => mockRepo.getLocalHighlightsDelta(any())).thenAnswer((_) async => mockHighlightDelta);
    ...
    verify(() => mockClient.pushHighlights(any(that: isA<List<Map<String, dynamic>>>().having(
      (list) => list.first['user_id'], 'secured user_id', 'mock-user-123'
    )))).called(1);
  ...
  test('SyncController sets error state and handles exception gracefully on network failure', () async {
    final controller = container.read(syncControllerProvider.notifier);
    
    // Force a network failure exception
    when(() => mockClient.signInAnonymously()).thenThrow(Exception('Supabase connection lost'));
    ...
    expect(finalState.lastSyncedAt?.millisecondsSinceEpoch, 1000); // Preserves previous anchor
  });
}
