...
void main() {
  group('PaperPhysicsEngine', () {
    ...
    test('zero dx produces minimal output', () {
      final engine = PaperPhysicsEngine(pageNumber: 1);
      final frame = engine.calculate(dx: 0.0, foldAngle: 0.0, screenWidth: 400.0);
    ...
    test('large dx produces higher values', () {
      final engine = PaperPhysicsEngine(pageNumber: 1);
      final frameSmall = engine.calculate(dx: 5.0, foldAngle: 0.5, screenWidth: 400.0);
      final frameLarge = engine.calculate(dx: 100.0, foldAngle: 0.5, screenWidth: 400.0);
      expect(frameLarge.amplitude, greaterThan(frameSmall.amplitude));
      expect(frameLarge.rawFriction, lessThan(frameSmall.rawFriction)); // Stribeck: velocity → lower friction
    });

    test('negative dx produces same result as positive', () {
      final engine = PaperPhysicsEngine(pageNumber: 1);
      final pos = engine.calculate(dx: 10.0, foldAngle: 0.5, screenWidth: 400.0);
      final neg = engine.calculate(dx: -10.0, foldAngle: 0.5, screenWidth: 400.0);
    ...
    test('consecutive calls accumulate noise position', () {
      final engine = PaperPhysicsEngine(pageNumber: 1);
      final frame1 = engine.calculate(dx: 20.0, foldAngle: 0.5, screenWidth: 400.0);
      final frame2 = engine.calculate(dx: 20.0, foldAngle: 0.5, screenWidth: 400.0);
    ...
    test('reset clears accumulated state', () {
      ...
      engine.calculate(dx: 20.0, foldAngle: 0.5, screenWidth: 400.0);
      engine.reset();
      final afterReset = engine.calculate(dx: 20.0, foldAngle: 0.5, screenWidth: 400.0);
    ...
    test('different page numbers produce different seeds', () {
      ...
      final frame1 = engine1.calculate(dx: 10.0, foldAngle: 0.5, screenWidth: 400.0);
      final frame2 = engine2.calculate(dx: 10.0, foldAngle: 0.5, screenWidth: 400.0);
    ...
    test('high fold angle produces higher resistance', () {
      final engine = PaperPhysicsEngine(pageNumber: 1);
      final lowAngle = engine.calculate(dx: 10.0, foldAngle: 0.0, screenWidth: 400.0);
      final highAngle = engine.calculate(dx: 10.0, foldAngle: 1.0, screenWidth: 400.0);
      expect(highAngle.rawResistance, greaterThan(lowAngle.rawResistance));
    });

    test('very small screenWidth produces clamped velocity', () {
      final engine = PaperPhysicsEngine(pageNumber: 1);
      final frame = engine.calculate(dx: 100.0, foldAngle: 0.5, screenWidth: 1.0);
    ...
    test('output ranges are bounded', () {
      ...
      for (final dx in [0.0, 1.0, 10.0, 50.0, 200.0]) {
        final frame = engine.calculate(dx: dx, foldAngle: 0.5, screenWidth: 400.0);
